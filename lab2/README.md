...
```
gcc -o dice_simulation dice_simulation.c -pthread -O2
```
**Флаги компиляции:**
- `-pthread` — подключает библиотеку POSIX Threads
- `-O2` — включает оптимизацию второго уровня для увеличения производительности
...
````
./dice_simulation 10 5 30 25 10000000 1
````
## Узнать количество логических ядер
````
./dice_simulation 10 5 30 25 100000000 8 &
````
Получить PID процесса
````
PID=$(pgrep dice_simulation)
````

Показать все потоки (замени <PID> на реальный номер процесса)
````
ps -T -p $PID
````